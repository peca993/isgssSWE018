...
export default class IzmenaAkcije extends Component {
    ...
    constructor(){
        ...
        this.state = {
            id: akcija.id,
            clanovi: neucesnici,
            clanovi_akcije: ucesnici,
            naziv: akcija.naziv,
            datum: akcija.datum,
            lokacija: akcija.lokacija,
            opis: akcija.opis,
    ...
    renderClanoviAkije(){
        ...
        <tr className="table" key={item.id} value={item.id} ><td>{item.name}</td><td>{item.prezime}</td><td>{item.nadimak}</td><td><button className="btn btn-danger" onClick={this.removeFromList.bind(this,item.id)} ><i class="fa fa-trash-o"></i></button></td></tr>
        );
        return (
        ...
        return (
            ...
            {console.log(this.danasnjiDatum())}
                ...
                <div className="form-group row">
                    ...
                    <div className="col-10">
                        <textarea value={this.state.opis} onChange={this.changeOpis} className="form-control" type="text"  name="opis" >
                        </textarea>
                    </div>
                </div>
                <div className="form-group row">
                    ...
                    <div className="" >
                        <div className="col-md-6">
                            <div className="row">
                                {this.renderLista()}
                                <button className="btn btn-primary col-3 " onClick={this.dodajClana}>Dodaj</button>        
                            </div>
                        </div>
                        <br/>
                        <div className="">
                    ...
                    <button className="btn btn-primary btn-lg btn-block" onClick={this.submit} >Izmeni</button>
                </div>
            </div>            
        );
    }
...
if (document.getElementById('izmena_akcije')) {
    ReactDOM.render(<IzmenaAkcije />, document.getElementById('izmena_akcije'));
}